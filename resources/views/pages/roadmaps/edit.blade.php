  ...
  <div class="flex min-h-screen">
    ...
    <main class="flex-1 bg-slate-50 p-8 overflow-auto">
      ...
      <div class="flex items-center justify-between mb-8">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Edit Roadmap: {{ $roadmap->title }}</h1>
          <p class="text-sm text-slate-500 mt-0.5">Update the learning path details and assign courses.</p>
        </div>
        <a href="{{ route('admin.roadmaps') }}" class="flex items-center gap-2 border border-slate-200 bg-white hover:border-slate-300 transition-colors text-slate-700 text-sm font-semibold px-4 py-2.5 rounded-lg">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
          Cancel
        </a>
      </div>

      <form method="POST" action="{{ route('roadmaps.update', $roadmap->id) }}" class="bg-white border border-slate-200 rounded-xl shadow-sm p-7">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          ...
          <div>
            ...
            <input id="url" name="url" type="url" value="{{ old('url', $roadmap->url) }}" placeholder="e.g. https://roadmap.sh/frontend" class="w-full px-3.5 py-2.5 text-sm border @error('url') border-rose-500 @else border-slate-200 @enderror rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition" />
        ...
        <div class="border-t border-slate-100 pt-5 mb-8">
          ...
          <label class="flex items-center gap-3 cursor-pointer select-none">
            <div class="relative">
              ...
              <input type="checkbox" name="is_published" value="1" id="is_published" class="sr-only peer" {{ old('is_published', $roadmap->is_published) ? 'checked' : '' }} />
            ...
            <div>
              ...
              <p class="text-xs text-slate-400">Make this path visible to students</p>
            </div>
          </label>
        </div>
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @foreach($courses as $course)
                            @php
                                // Check if this course is already attached to this roadmap
                                $isAttached = $roadmap->courses->contains($course->id);
                                // If attached, grab the current order, otherwise default to 0
                                $currentOrder = $isAttached ? $roadmap->courses->find($course->id)->pivot->sort_order : 0;
                            @endphp
                            <tr class="hover:bg-white transition-colors {{ $isAttached ? 'bg-indigo-50/30' : '' }}">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <input type="number" 
                                           ...
                                           value="{{ old('course_orders.' . $course->id, $currentOrder) }}"
                                           min="0"
