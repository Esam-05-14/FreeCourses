  ...
  <div class="flex min-h-screen">
    ...
    <main class="flex-1 bg-slate-50 p-8 overflow-auto">
      ...
      <div class="flex items-center gap-3 mb-6">
        <form action="{{ route('admin.roadmaps') }}" method="GET" class="flex items-center gap-3 flex-1">
        ...
        <div class="flex-1 relative">
          ...
          <input type="text" placeholder="Search roadmaps…" name="search" value="{{ request('search') }}"  class="w-full pl-9 pr-4 py-2.5 text-sm border border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
        </div>
        ...
        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2.5 rounded-lg">
          Filter
        </button>
        @if(request()->anyFilled(['search', 'is_published']))
        <a href="{{ route('admin.roadmaps') }}" class="text-slate-500 hover:text-rose-600 text-sm font-medium px-2 py-2.5 transition-colors">
            Clear
        </a>
    @endif
        </form>
