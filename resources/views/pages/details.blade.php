  ...
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
      ...
      <div class="flex items-center gap-3 shrink-0">
          <form action="{{ route('courses.start', $course->id) }}" method="POST" class="inline-block">
              @csrf
              <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-3 rounded-lg transition-colors flex items-center gap-2">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                  Start Course Now
              </button>
          </form>

          <form action="{{ route('courses.save', $course->id) }}" method="POST" class="inline-block ml-3">
              @csrf
              <button type="submit" class="bg-white border border-slate-200 text-slate-700 hover:border-slate-300 hover:bg-slate-50 font-semibold px-6 py-3 rounded-lg transition-colors flex items-center gap-2">
