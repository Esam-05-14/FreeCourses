  ...
  <div class="flex min-h-screen">
    ...
    <main class="flex-1 bg-slate-50 p-8 overflow-auto">
      <div class="flex items-center justify-between mb-8">
        ...
        <a href="{{ route('courses.index') }}" class="flex items-center gap-2 border border-slate-200 bg-white hover:border-slate-300 transition-colors text-slate-700 text-sm font-semibold px-4 py-2.5 rounded-lg">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
          Cancel
        </a>
      </div>

      <form method="POST" action="{{ route('courses.store') }}" class="bg-white border border-slate-200 rounded-xl shadow-sm p-7">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          <div>
            ...
            <input id="title" name="title" value="{{ old('title') }}" type="text" placeholder="e.g. The Complete JavaScript Course 2024" class="w-full px-3.5 py-2.5 text-sm border @error('title') border-rose-500 @else border-slate-200 @enderror rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition" />
          ...
          <div>
            ...
            <input id="slug" name="slug" value="{{ old('slug') }}" type="text" placeholder="e.g. complete-javascript-course-2024" class="w-full px-3.5 py-2.5 text-sm border @error('slug') border-rose-500 @else border-slate-200 @enderror rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          <div>
            ...
            <input id="url" name="url" value="{{ old('url') }}" type="url" placeholder="https://youtube.com/watch?v=..." class="w-full px-3.5 py-2.5 text-sm border @error('url') border-rose-500 @else border-slate-200 @enderror rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition" />
          ...
          <div>
            ...
            <select id="provider" name="provider" class="w-full px-3.5 py-2.5 text-sm border @error('provider')  @else border-slate-200 @enderror rounded-lg text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
              ...
              <option value="Udemy" {{ old('provider') == 'Udemy' ? 'selected' : '' }}>Udemy</option>
              <option value="FreeCodeCamp" {{ old('provider') == 'FreeCodeCamp' ? 'selected' : '' }}>FreeCodeCamp</option>
            </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          <div>
            ...
            <input id="duration" name="duration" value="{{ old('duration') }}" type="number" min="0" placeholder="e.g. 4140" class="w-full px-3.5 py-2.5 text-sm border @error('duration') border-rose-500 @else border-slate-200 @enderror rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition" />
        ...
        <div class="mb-5">
          ...
          <textarea id="description" name="description" rows="4" placeholder="Write a brief description of the course content..." class="w-full px-3.5 py-2.5 text-sm border @error('description') border-rose-500 @else border-slate-200 @enderror rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition resize-none">{{ old('description') }}</textarea>
          ...
          <div class="flex items-center gap-8">
            <label class="flex items-center gap-3 cursor-pointer select-none">
              <div class="relative">
                ...
                <input type="checkbox" name="is_featured" value="1" id="is_featured" class="sr-only peer" {{ old('is_featured') ? 'checked' : '' }} />
            ...
            <label class="flex items-center gap-3 cursor-pointer select-none">
              <div class="relative">
                ...
                <input type="checkbox" name="is_published" value="1" id="is_published" class="sr-only peer" {{ old('_token') === null || old('is_published') ? 'checked' : '' }} />
                <div class="w-10 h-5 bg-slate-200 rounded-full peer peer-checked:bg-indigo-600 transition-colors"></div>
                <div class="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform peer-checked:translate-x-5"></div>
              </div>
              <div>
        ...
        <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-5">
          ...
          <button type="submit" name="action" value="publish" class="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 transition-colors text-white text-sm font-semibold px-6 py-2.5 rounded-lg">
